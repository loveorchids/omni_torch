...
class InceptionNet_V3(nn.Module):
    def __init__(self, BN, num_classes=1000, aux_logits=True):
        ...
        self.conv_layers_1 = omth_blocks.conv_block(3, filters=[32, 32, 64], kernel_sizes=[3, 3, 3], stride=[2, 1, 1],
                                                  padding=[0, 0, 1], name="conv_block_1", batch_norm=BN, bn_eps=1e-3)
        self.conv_layers_2 = omth_blocks.conv_block(64, filters=[80, 192], kernel_sizes=[1, 3], stride=[1, 1],
                                                  padding=[0, 0], name="conv_block_2", batch_norm=BN, bn_eps=1e-3)
        self.Mixed5b = self.inceptionA(in_channels=192, pool_features=32, pool=self.avg_pool_3_1_1, bn_eps=1e-3)
        self.Mixed5c = self.inceptionA(in_channels=256, pool_features=64, pool=self.avg_pool_3_1_1, bn_eps=1e-3)
        self.Mixed5d = self.inceptionA(in_channels=288, pool_features=64, pool=self.avg_pool_3_1_1, bn_eps=1e-3)
        self.Mixed6a = omth_blocks.InceptionBlock(288, filters=[[384], [64, 96, 96]], kernel_sizes=[[3], [1, 3, 3]],
                                                  stride=[[2], [1, 1, 2]], padding=[[0], [0, 1, 0]],
                                                  batch_norm=BN, bn_eps=1e-3,  maxout=self.pool_3_2_0)
        self.Mixed6b = self.inceptionC(in_channels=768, c7=120, pool=self.avg_pool_3_1_1, bn_eps=1e-3)
        self.Mixed6c = self.inceptionC(in_channels=768, c7=160, pool=self.avg_pool_3_1_1, bn_eps=1e-3)
        self.Mixed6d = self.inceptionC(in_channels=768, c7=160, pool=self.avg_pool_3_1_1, bn_eps=1e-3)
        self.Mixed6e = self.inceptionC(in_channels=768, c7=192, pool=self.avg_pool_3_1_1, bn_eps=1e-3)
        ...
        if aux_logits:
            self.aux_conv = omth_blocks.conv_block(768, filters=[128, 768], kernel_sizes=[1, 5], stride=[1, 1],
                                                   padding=[0, 0], batch_norm=BN, bn_eps=1e-3)
            self.aux_conv.stdev = 0.01
            self.aux_fc = nn.Linear(768, num_classes)
            self.aux_fc.stddev = 0.001

    def inceptionA(self, in_channels, pool_features, pool, bn_eps):
        return omth_blocks.InceptionBlock(in_channels, filters=[[64], [48, 64], [64, 96, 96], [pool_features]],
                                          ...
                                          bn_eps=bn_eps, inner_maxout=[None, None, None, pool])
    def inceptionC(self, in_channels, c7, pool, bn_eps):
        return omth_blocks.InceptionBlock(in_channels, filters=[[192], [c7, c7, 192], [c7, c7, c7, c7, 192], [192]],
                                          ...
                                          batch_norm=self.BN, bn_eps=bn_eps, inner_maxout=[None, None, None, pool])
...
class InceptionE(nn.Module):
    def __init__(self, in_channels, BN, bn_eps=1e-5):
        ...
        self.branch1x1 = omth_blocks.conv_block(in_channels, filters=[320], kernel_sizes=[1], stride=[1],
                                                padding=[0], batch_norm=BN, bn_eps=bn_eps)

        self.branch3x3_1 = omth_blocks.conv_block(in_channels, filters=[384], kernel_sizes=[1], stride=[1],
                                                  padding=[0], batch_norm=BN, bn_eps=bn_eps)
        self.branch3x3_2 = self.sub_inception_module(BN, bn_eps=bn_eps)

        self.branch3x3dbl_1 = omth_blocks.conv_block(in_channels, filters=[384, 384], kernel_sizes=[1, 3], stride=[1, 1],
                                                     padding=[0, 1], batch_norm=BN, bn_eps=bn_eps)
        self.branch3x3dbl_2 = self.sub_inception_module(BN, bn_eps=bn_eps)
    ...
    def sub_inception_module(self, BN, bn_eps=1e-5):
        return omth_blocks.InceptionBlock(384, filters=[384, 384], kernel_sizes=[[[1, 3]], [[3, 1]]],
                                          stride=[1, 1], padding=[[[0, 1]], [[1, 0]]], batch_norm=BN, bn_eps=bn_eps)
